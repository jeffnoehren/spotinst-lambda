...
function destroy(err, event, context) {
  ...
  spotUtil.getToken(event, function(err, token) {
    ...
    let createOptions = {
      ...
      url:     `https://api.spotinst.io/loadBalancer/routingRule/${refId}`,
      headers: {
    ...
    console.log(`Deleting routingRule  ${refId}`);
      ...
      spotUtil.validateResponse({
        ...
        failureCb: function(spotResponse) {
          ...
          if(spotResponse.res.statusCode === 404) {
            console.log("The routingRule doesn't exist, set the delete as success.");
            util.done(null, event, context, "The routingRule doesn't exist");
          }
          else {
