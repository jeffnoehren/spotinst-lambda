...
function destroy(err, event, context) {
  ...
  spotUtil.getToken(event, function(err, token) {
    ...
    let createOptions = {
      ...
      url:     `https://api.spotinst.io/loadBalancer/deployment/${refId}`,
      headers: {
    ...
    console.log(`Deleting deployment  ${refId}`);
      ...
      spotUtil.validateResponse({
        ...
        failureCb: function(spotResponse) {
          ...
          if(spotResponse.res.statusCode === 404) {
            console.log("The deployment doesn't exist, set the delete as success.");
            util.done(null, event, context, "The deployment doesn't exist");
          }
          else {
