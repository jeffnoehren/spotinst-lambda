...
function destroy(err, event, context) {
  ...
  spotUtil.getToken(event, function(err, token) {
    ...
    let createOptions = {
      ...
      url:     `https://api.spotinst.io/loadBalancer/listener/${refId}`,
      headers: {
    ...
    console.log(`Deleting listener  ${refId}`);
      ...
      spotUtil.validateResponse({
        ...
        failureCb: function(spotResponse) {
          ...
          if(spotResponse.res.statusCode === 404) {
            console.log("The listener doesn't exist, set the delete as success.");
            util.done(null, event, context, "The listener doesn't exist");
          }
          else {
