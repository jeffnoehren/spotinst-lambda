...
function destroy(err, event, context) {
  ...
  spotUtil.getToken(event, function(err, token) {
    ...
    let createOptions = {
      ...
      url:     `https://api.spotinst.io/loadBalancer/targetSet/${refId}`,
      headers: {
    ...
    console.log(`Deleting targetSet  ${refId}`);
      ...
      spotUtil.validateResponse({
        ...
        failureCb: function(spotResponse) {
          ...
          if(spotResponse.res.statusCode === 404) {
            console.log("The targetSet doesn't exist, set the delete as success.");
            util.done(null, event, context, "The targetSet doesn't exist");
          }
          else {
